...
\begin{document}
	...
	\chapter{Nền tảng lý thuyết: Hệ thống Naive RAG và Giới hạn cốt lõi}
	\label{chap:naive_rag}

	\section{Tại sao cần RAG? Hạn chế của Mô hình Ngôn ngữ Lớn}
	\label{sec:llm_limitations}

	Khi bắt đầu xây dựng hệ thống tư vấn tuyển sinh cho Trường Đại Học Khoa Học
	-- Đại Học Huế, câu hỏi đầu tiên đặt ra là: liệu có thể dùng thẳng một mô
	hình ngôn ngữ lớn như GPT hay Gemini để trả lời các câu hỏi về tuyển sinh
	không? Câu trả lời là không thể, và nguyên nhân không nằm ở chất lượng của
	mô hình mà nằm ở bốn hạn chế có tính cấu trúc mà bất kỳ LLM nào cũng mang
	theo~\cite{neo4j2024essential}.
	...
	% -------------------------------------------------------------------
	\section{Hệ thống PaddedRAG: Kiến trúc Baseline và Pipeline Truy xuất Lai}
	\label{sec:paddedrag_baseline}

	Trước khi phân tích kiến trúc GraphRAG ở Chương~\ref{chap:graphrag_theory},
	cần thiết lập rõ ràng hệ thống baseline mà chúng tôi đã xây dựng và vận hành
	thực tế --- một hệ thống Naive RAG được tăng cường với nhiều kỹ thuật hiện đại,
	mà trong nghiên cứu này chúng tôi đặt tên là \emph{PaddedRAG}. Tên gọi này
	phản ánh ý tưởng trung tâm: bù đắp (``pad'') cho sự thiếu hụt ngữ cảnh của
	chunking tĩnh bằng các lớp xử lý bổ sung, thay vì thay đổi cấu trúc dữ liệu
	nền tảng.

	\subsection{Kiến trúc Tổng thể}
	\label{subsec:paddedrag_architecture}

	Pipeline PaddedRAG bao gồm năm giai đoạn nối tiếp, mỗi giai đoạn được thiết
	kế để khắc phục một điểm yếu cụ thể của Naive RAG thuần túy.

	\textbf{Giai đoạn 1 --- Tăng cường truy vấn (Query Enhancement).}
	Thay vì truy xuất trực tiếp với câu hỏi gốc $q$, hệ thống áp dụng kỹ thuật
	HyDE (Hypothetical Document Embeddings): LLM được yêu cầu tạo ra một tài liệu
	giả định $\hat{d}$ mà nếu tồn tại sẽ trả lời được $q$. Trực giác nằm ở
	chỗ: không gian vector của câu hỏi và câu trả lời thường không trùng nhau;
	một tài liệu giả định ``đứng ở phía câu trả lời'' giúp rút ngắn khoảng cách
	này. Trong thực tế, chúng tôi sử dụng Groq (Llama 3.1) làm primary LLM
	với GLM-4 và Gemini-2.0-flash là hai fallback nhằm đảm bảo tính sẵn sàng
	(availability) của dịch vụ.

	\textbf{Giai đoạn 2 --- Truy xuất song song (Parallel Retrieval).}
	Từ truy vấn tăng cường, hệ thống thực hiện hai luồng tìm kiếm đồng thời:
	\emph{(a)} Dense retrieval qua không gian vector với mô hình nhúng
	\texttt{BAAI/bge-m3} (1024 chiều, đa ngôn ngữ, hỗ trợ tiếng Việt), truy
	xuất $K_{\text{dense}} = 20$ ứng viên hàng đầu; và \emph{(b)} Sparse retrieval
	qua BM25 (tokenized theo ký tự n-gram để xử lý tiếng Việt không dấu), truy
	xuất $K_{\text{sparse}} = 20$ ứng viên.

	\textbf{Giai đoạn 3 --- Hợp nhất RRF (Reciprocal Rank Fusion).}
	Kết quả từ hai luồng được hợp nhất theo thuật toán RRF
	(xem Công thức~\eqref{eq:rrf} ở Chương~\ref{chap:graphrag_theory}) với
	trọng số $w_{\text{dense}} = 0{,}6$ và $w_{\text{sparse}} = 0{,}4$, hằng số
	$k_0 = 60$. Trọng số bất đối xứng này phản ánh thực tế rằng BGE-M3 có khả
	năng hiểu ngữ nghĩa tiếng Việt tốt hơn BM25 --- đặc biệt với các câu hỏi
	paraphrase hoặc câu hỏi có thuật ngữ hành chính đặc thù.

	\textbf{Giai đoạn 4 --- Xếp hạng lại (Cross-Encoder Reranking).}
	Top-50 ứng viên từ RRF được đưa qua Cross-Encoder
	\texttt{BAAI/bge-reranker-base} để tính điểm liên quan chính xác hơn. Khác
	với Bi-Encoder (mã hóa câu hỏi và tài liệu độc lập), Cross-Encoder nhận
	đầu vào là cặp $(q, d)$ và tính điểm liên quan qua attention hai chiều --- cho
	phép nắm bắt sự tương tác từ-với-từ tinh tế hơn, với chi phí là độ trễ
	tỉ lệ tuyến tính với số ứng viên.

	\textbf{Giai đoạn 5 --- Lọc ngưỡng và Tạo sinh.}
	Sau reranking, hệ thống giữ lại các chunk có điểm vượt ngưỡng thích nghi
	$\tau_{\text{conf}}$: $\tau = 0{,}35$ với corpus nhỏ dưới 500 tài liệu,
	$\tau = 0{,}45$ trong khoảng $500$--$5000$, và $\tau = 0{,}55$ với corpus
	lớn hơn. Top-3 chunk sau lọc được nối vào prompt của LLM tạo sinh
	(Gemini-2.0-flash hoặc GLM-4.5 tùy availability). Ngưỡng thích nghi này
	là một heuristic thực tiễn: corpus lớn hơn thường dẫn đến nhiều ứng viên
	``tương tự đủ'' nhưng không thực sự liên quan, nên ngưỡng cao hơn là cần
	thiết để giữ precision.
	...
	\begin{figure}[htbp]
		...
		\begin{adjustbox}{max width=\linewidth}
			\begin{tikzpicture}[
				...
				stage/.style={rectangle, rounded corners=4pt, draw=black!65,
				...
				\node[stage, fill=purple!15, draw=purple!50] (q)   at (0,0)    {Truy vấn\\người dùng $q$};
				\node[stage, fill=orange!15] (hyde) at (2.8,0)  {HyDE\\(Groq/GLM-4)};
				\node[stage, fill=blue!15]   (dens) at (5.6,1)  {Dense\\BGE-M3\\$K{=}20$};
				\node[stage, fill=green!15]  (bm25) at (5.6,-1) {Sparse\\BM25\\$K{=}20$};
				\node[stage, fill=yellow!20, draw=yellow!60!black] (rrf) at (8.4,0) {RRF\\$w_d{=}0.6$\\$w_s{=}0.4$};
				\node[stage, fill=red!12]    (rer)  at (11.2,0) {Cross-Encoder\\Reranker\\Top-50};
				\node[stage, fill=green!20, draw=green!55!black] (gen) at (14,0) {LLM\\Generator\\(Gemini)};

				\draw[arrow] (q) -- (hyde);
				\draw[arrow] (hyde) |- (dens);
				\draw[arrow] (hyde) |- (bm25);
				\draw[arrow] (dens) -| (rrf);
				\draw[arrow] (bm25) -| (rrf);
				\draw[arrow] (rrf) -- (rer);
				\draw[arrow] (rer) -- (gen);

				\node[label] at (0,-1.4)    {Đầu vào};
				\node[label] at (2.8,-1.4)  {Giai đoạn 1};
				\node[label] at (5.6,-1.8)  {Giai đoạn 2};
				\node[label] at (8.4,-1.4)  {Giai đoạn 3};
				\node[label] at (11.2,-1.4) {Giai đoạn 4};
				\node[label] at (14,-1.4)   {Giai đoạn 5};
			\end{tikzpicture}
		\end{adjustbox}
		\caption{Pipeline năm giai đoạn của hệ thống PaddedRAG baseline. Truy vấn
			được tăng cường qua HyDE trước khi kích hoạt hai luồng truy xuất song song
			(Dense và Sparse); kết quả hợp nhất qua RRF rồi xếp hạng lại bằng
			Cross-Encoder trước khi đưa vào LLM tạo sinh.}
		\label{fig:paddedrag_pipeline}
	\end{figure}

	\subsection{Điểm mạnh và Giới hạn Cấu trúc}
	\label{subsec:paddedrag_limits}

	Trong quá trình xây dựng và vận hành hệ thống, chúng tôi nhận thấy PaddedRAG
	thể hiện hiệu quả đáng kể trên một lớp câu hỏi quan trọng: các câu hỏi tra
	cứu thực thể đơn giản --- loại câu hỏi chỉ yêu cầu định vị một đoạn văn bản
	cụ thể. Ví dụ, ``Điểm chuẩn ngành Công nghệ Thông tin năm 2024 là bao nhiêu?''
	được trả lời chính xác gần như tuyệt đối, vì câu trả lời được gói gọn trong
	một chunk duy nhất và BM25 có thể định vị nó hiệu quả qua từ khóa.

	Tuy nhiên, khi gặp câu hỏi yêu cầu tổng hợp từ nhiều nguồn --- ``Trong số
	các ngành của Khoa CNTT, ngành nào có điểm chuẩn tăng liên tục ba năm gần
	nhất?'' --- PaddedRAG bộc lộ giới hạn cấu trúc không thể khắc phục bằng
	cách tinh chỉnh tham số. Thứ nhất, không có cơ chế nào trong pipeline
	trên đảm bảo rằng tất cả các chunk liên quan đến ``điểm chuẩn ngành X năm
	2022'', ``điểm chuẩn ngành X năm 2023'', và ``điểm chuẩn ngành X năm 2024''
	sẽ cùng xuất hiện trong top-3 kết quả. Thứ hai, ngay cả khi cả ba chunk đó
	được truy xuất, LLM vẫn phải tự thực hiện phép so sánh và suy luận --- một
	nhiệm vụ dễ dẫn đến ảo giác nếu thông tin không được trình bày nhất quán.

	Bảng~\ref{tab:paddedrag_config} tóm tắt các siêu tham số chính của hệ thống
	PaddedRAG được sử dụng làm baseline cho thực nghiệm tại Chương~\ref{chap:experiments}.

	\begin{table}[htbp]
		\centering
		\caption{Cấu hình siêu tham số hệ thống PaddedRAG (baseline)}
		\label{tab:paddedrag_config}
		\begin{tabular}{lll}
			\toprule
			\textbf{Thành phần}       & \textbf{Tham số}              & \textbf{Giá trị}            \\
			\midrule
			Embedding                 & Mô hình                       & \texttt{BAAI/bge-m3}        \\
			                          & Chiều vector                  & 1024                        \\
			                          & Chuẩn hóa                    & L2-normalize                \\
			\midrule
			Chunking                  & Kích thước chunk $\ell$       & 350 tokens                  \\
			                          & Độ chồng lấp (overlap)        & 70 tokens                   \\
			\midrule
			Truy xuất                 & Top-K dense                   & 20                          \\
			                          & Top-K sparse (BM25)           & 20                          \\
			                          & Top-K sau reranking           & 50                          \\
			\midrule
			RRF                       & $w_{\text{dense}}$            & 0,6                         \\
			                          & $w_{\text{sparse}}$           & 0,4                         \\
			                          & $k_0$                         & 60                          \\
			\midrule
			Reranker                  & Mô hình                       & \texttt{BAAI/bge-reranker-base} \\
			\midrule
			Ngưỡng tin cậy            & $\tau$ (corpus $<$500)        & 0,35                        \\
			                          & $\tau$ (corpus 500--5000)     & 0,45                        \\
			                          & $\tau$ (corpus $>$5000)       & 0,55                        \\
			\midrule
			LLM                       & Primary                       & Gemini-2.0-flash-exp        \\
			                          & Fallback                      & GLM-4.5                     \\
			                          & Temperature                   & 0,1                         \\
			\bottomrule
		\end{tabular}
	...
	\subsection{HippoRAG 2: Phiên bản Cải tiến với Passage Node}
	\label{subsec:hipporag2}

	Một hạn chế của HippoRAG thế hệ đầu là PPR chỉ lan truyền điểm số qua các
	node thực thể, trong khi bản thân đoạn văn bản gốc (passage) --- nơi chứa
	ngữ cảnh cần thiết cho LLM --- lại không được tính điểm trực tiếp. HippoRAG
	2~\cite{gutierrez2024hipporag} giải quyết vấn đề này bằng cách mở rộng mô
	hình đồ thị: thêm các \emph{passage node} vào cùng đồ thị với entity node,
	kết nối qua cạnh \texttt{MENTIONED\_IN}.

	Thay đổi này có hệ quả quan trọng: điểm số PPR bây giờ lan truyền từ entity
	node sang passage node liên kết, cho phép hệ thống xếp hạng trực tiếp các
	đoạn văn bản thay vì phải thực hiện bước tra cứu trung gian. Ngoài ra,
	HippoRAG 2 tích hợp \emph{dense passage retrieval} như một luồng bổ sung
	song song với PPR, sau đó kết hợp hai điểm số qua RRF. Kết quả thực nghiệm
	trên bộ dữ liệu MuSiQue và 2WikiMultiHopQA cho thấy cải thiện 7--12\% so với
	phiên bản gốc trên câu hỏi đa bước, trong khi không làm suy giảm hiệu quả
	trên câu hỏi đơn giản --- một đặc tính quan trọng mà nhiều hệ thống GraphRAG
	phức tạp khác không đạt được.
	...
	\begin{thebibliography}{99}
		...
		In \textit{Advances in Neural Information Processing Systems} (NeurIPS), 37.
		...
		In \textit{Findings of the Association for Computational Linguistics: EMNLP 2025}.
		\url{https://aclanthology.org/2025.findings-emnlp.568/}

		\bibitem{tang2025graphragsurvey}
		Tang, S., et al. (2025).
		\textit{Graph Retrieval-Augmented Generation: A Survey}.
		\textit{ACM Transactions on Information Systems}.
		\url{https://dl.acm.org/doi/10.1145/3777378}

		\bibitem{luo2025hypergraphrag}
		Luo, H., et al. (2025).
		\textit{HyperGraphRAG: Retrieval-Augmented Generation via Hypergraph-Structured Knowledge Representation}.
		In \textit{Advances in Neural Information Processing Systems} (NeurIPS), 38.
		\url{https://neurips.cc/virtual/2025/poster/115764}

		\bibitem{lee2025hybgrag}
		Lee, M.-C., Zhu, Q., Mavromatis, C., Han, Z., Adeshina, S., Ioannidis, V.~N.,
		Rangwala, H., \& Faloutsos, C. (2025).
		\textit{HYBGRAG: Hybrid Retrieval-Augmented Generation on Textual and Relational Knowledge Bases}.
		In \textit{Proceedings of the 63rd Annual Meeting of the Association for Computational Linguistics}
		(ACL 2025), 879--893.
		\url{https://aclanthology.org/2025.acl-long.43/}

		\bibitem{yu2025graphflow}
		Yu, J., Liu, Y., Gu, J., Torr, P., \& Zhou, D. (2025).
		\textit{Can Knowledge-Graph-based Retrieval Augmented Generation Really Retrieve What You Need?}
		In \textit{Advances in Neural Information Processing Systems} (NeurIPS 2025, Spotlight).
		\url{https://neurips.cc/virtual/2025/poster/115922}

		\bibitem{kim2025regraphrag}
		Kim, S., Hwang, S.~J., Kim, J., Park, J., \& Choi, Y.~S. (2025).
		\textit{ReGraphRAG: Reorganizing Fragmented Knowledge Graphs for
		Multi-Perspective Retrieval-Augmented Generation}.
		In \textit{Findings of the Association for Computational Linguistics: EMNLP 2025},
		5426--5443.
