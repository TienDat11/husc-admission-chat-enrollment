...
echo.
echo   %GREEN%1. apply_config.bat%RESET%    - Fix configuration (.env template)
echo   %GREEN%2. setup_data.bat%RESET%      - Normalize → validate → chunk → LanceDB → Graph
echo   %GREEN%3. python -m uvicorn src.main:app --host 0.0.0.0 --port 8000 --reload%RESET%
echo.
...
echo.
echo   %CYAN%setup_data.bat%RESET%         - Full pipeline (validate → chunk → LanceDB → GraphRAG)
echo                            Input:  ../2.jsonl
echo                            Output: data\lancedb\ + data\graph\knowledge_graph.graphml
echo.
...
echo     python scripts\ingest_lancedb.py
echo     python scripts\build_graph.py --incremental
echo     python -m uvicorn src.main:app --host 0.0.0.0 --port 8000 --reload
echo.
echo   %YELLOW%Troubleshooting:%RESET%
echo     python scripts\preflight_check.py  (check system health)
echo     type .env ^| findstr "USE_LANCEDB LANCEDB_TABLE EMBEDDING_DIM"  (verify config)
echo     python scripts\check_lancedb.py  (verify LanceDB exists)
echo.
